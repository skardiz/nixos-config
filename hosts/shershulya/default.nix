...

{
  imports = [
    ...
    secrets.github_token = {
      ...
      # Мы убираем 'group' и 'neededForUsers', так как они не решили проблему.
      # Вместо этого мы используем более фундаментальный подход:
      # Мы явно указываем, что этот секрет нужен для пользователя 'root',
      # от имени которого запускается процесс сборки. Это гарантирует,
      # что ключ будет доступен в нужный момент.
